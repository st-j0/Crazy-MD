...
if(res.status===200){ status = `_Supprimé avec succès ${a} var from koyeb._`} else {status = '_Veuillez entre votro API koyeb dans var KOYEB_API._\nEx: KOYEB_API:api key'}
})
return status
}
//----------------------------------------------------------------------------------------------------------------------------------------------------
async function change_env(a){
var status = '_Veuillez entre votro API koyeb dans var KOYEB_API._\nEx: KOYEB_API:api key'
let { data } = await axios.get(`https://app.koyeb.com/v1/services`,axiosConfig)
 let ser = data.services[0].id
let b = await axios.get(`https://app.koyeb.com/v1/deployments/${data.services[0].latest_deployment_id}`,axiosConfig)
let bb = a.split(':')
...
let body = {
 ...
 await axios.patch(`https://app.koyeb.com/v1/services/${ser}`, body, axiosConfig).then(res => {
            if(res.status===200){status = ` var changée avec Succès _${bb[0]}:${bb[1]} ._` } else {status = '_Veuillez entre votro API koyeb dans var KOYEB_API._\nEx: KOYEB_API:api key'}
          })
 return status
}
...
k = '_update started._'
  ...
  k = '*Erreur lors du redeploiement.*\n*Veuillez entre votro API koyeb dans var KOYEB_API.*\n_Ex: KOYEB_API:api key de https://app.koyeb.com/account/api ._'
}
return k
}
