...
const { Sticker, createSticker, StickerTypes } = require("wa-sticker-formatter");
    ...
    async(Void, citel, text) => {
        ...
        if (!citel.quoted) return citel.reply(`_Taggez un Stickers._`)
...
} else return citel.reply ("```Veuillez tagger un Stickers animé```")
    }
)
//---------------------------------------------------------------------------
...
if(!citel.quoted) return citel.reply("```Veuillez tagger un message Vue unique```")           
...
else return citel.reply("```MRRD C'est pas un message Vue unique```")
...
cmd({
        ...
        async(Void, citel, text) => {
            if (!citel.quoted) return citel.reply("Veuiller tagger ou citer un message/texte");
...
cmd({
        ...
        async(Void, citel, text) => {
            if (isNaN(text.split(" ")[0]) || !text) {
                let text = tiny(
                    "Generateur de style de texte Fancy\n\nEx: .fancy 32 CRAZYMD\n\n"
                );
...
cmd({
        ...
        async(Void, citel, text) => {
            if (!text) return citel.reply('ENVOYEZ UN LIEN')
            try {
                link = text.split(" ")[0];
                anu = await axios.get(`https://tinyurl.com/api-create.php?url=${link}`);
                citel.reply(`*🛡️Votre lien raccourcis*\n\n${anu.data}`);
            } catch (e) {
                console.log(e);
            }
        }
...
filename: __filename,
    ...
    async(Void, citel, text) => {
        if (!citel.quoted) return citel.reply(`*Veuillez tagger une image/video, Mr.*`);
      //console.log("Quoted Data here : ",citel.quoted);
       ...
       if (mime =="imageMessage" || mime =="stickerMessage") {
        ...
        }else return citel.reply("*Mrrd, veuillez tagger une image*");
...
filename: __filename,
    ...
    async(Void, citel, text) => {
        if (!citel.quoted) return citel.reply(`*Veuillez taager une image/video Svp.*`);
      //console.log("Quoted Data here : ",citel.quoted);
        let mime = citel.quoted.mtype
        pack = Config.packname
        author = Config.author
        if (mime =="imageMessage"  || mime =="stickerMessage") {
            ...
            const buffer = await sticker.toBuffer();
            return Void.sendMessage(citel.chat, {sticker: buffer}, {quoted: citel });
        }else return citel.reply("*MRR, tagez une image*");
...
async(Void, citel, text) => {
    if (!citel.quoted) return citel.reply(`_Veuillez tagger une video_`);
    let mime = citel.quoted.mtype
...
else return citel.reply ("```_Taggez un message video Svp```")
}
)